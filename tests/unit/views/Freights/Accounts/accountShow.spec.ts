...
describe('FreightAccountShow.vue', () => {
    beforeEach(async () => {
    ...
    it('renders freight account show vue', async () => {
        const accountStore = useAccountsStore();
        const freightsStore = useFreightsStore();

        jest.spyOn(freightsStore, 'findFreight').mockResolvedValueOnce(freight);

        jest.spyOn(accountStore, 'findAccount').mockResolvedValueOnce(
            accountsInitialState()._newItem
        );

        const wrapper = mount(FreightAccountShow, {
            global: {
